...
- **Path-mode previews share warren's browser origin.** `WARREN_PREVIEW_MODE=path` is the default. It puts a run's preview app on `/p/<run-id>/`, the same browser origin as the warren UI. The operator bearer token sits in `localStorage` on that origin. An agent writes the preview app, so a script in it can read that token and then drive the control plane as the operator. The proxy strips `Authorization` and `Cookie` before it forwards a request upstream, so the preview process never sees warren credentials. Only the browser side stays open. Mitigation: choose `WARREN_PREVIEW_MODE=subdomain` when a preview may run code you would not run yourself. Subdomain mode gives each preview its own browser origin, so the preview cannot reach the UI storage. No response header can retrofit that boundary onto path mode. A `sandbox` CSP directive does give the preview an opaque origin. It also makes the browser treat the preview sub-resource requests as cross-site, which blocks the `SameSite=Lax` preview cookie those requests need.
